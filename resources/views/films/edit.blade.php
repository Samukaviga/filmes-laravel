        ...
        <div class="flex flex-col items-center h-full w-fill p-5">
            ...
            @if(session('success'))
                <div id="alert-3" class="flex items-center p-4 mb-4 text-green-800 rounded-lg bg-green-200" role="alert">
                    ...
                    <button type="button"
                        class="ms-auto -mx-1.5 -my-1.5 ml-4 bg-green-200 text-green-500 hover:text-green-600 rounded-lg focus:ring-2 focus:ring-green-400 p-1.5 hover:bg-green-300 inline-flex items-center justify-center h-8 w-8"
            ...
            @if($errors->any())
                @foreach ($errors->all() as $error)
                    <div class="flex items-center p-4 mb-4 text-red-800 rounded-lg bg-red-50" role="alert">
                        ...
                        <button type="button"
                            class="ms-auto bg-red-50 text-red-500 rounded-lg p-1.5 hover:bg-red-200 focus:ring-2 focus:ring-red-400"
                            aria-label="Fechar">
            ...
            <form action="{{ route('film.update', $film->id) }}" method="POST" class="w-full border border-gray-700 rounded-md p-5"
                enctype="multipart/form-data">
                @csrf
                <div class="mb-5">
                    <label for="large-input" class="block mb-2 text-sm font-medium text-gray-300">Nome</label>
                    <input type="text" id="large-input" name="name" value="{{ $film->name }}"
                ...
                <div class="mb-5">
                    ...
                    <select name="category" id="countries"
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" {{ $film->category->id == $category->id ? 'selected' : ''}}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                ...
                <div class="mb-5">
                    <label class="block mb-2 text-sm font-medium text-gray-300">Capa atual</label>
                    <img src="{{ asset('storage/' . $film->image) }}" alt="Capa do filme"
                        class="w-28 h-auto object-contain rounded mb-2">
                </div>

                <div class="mb-5">
                    <label class="block mb-2 text-sm font-medium text-gray-300" for="file_input">Trocar capa do
                        filme</label>
                    <input
